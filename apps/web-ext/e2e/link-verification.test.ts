...
const WARNING_TITLE = /Unable to verify site identity|サイトの身元を確認できません/;
...
test.describe("リンク検証", () => {
    ...
    test("Warning → Proceed anyway で遷移先に進む", async ({
        context,
        page,
    }) => {
        await page.goto(VERIFY_LINK_PAGE);
        ...
        await page.waitForURL(
            (url) => url.href.includes("example.com"),
            { timeout: 10_000 },
        );
    });

    test("Warning ページに正しい情報が表示される", async ({
        context,
        page,
    }) => {
        await page.goto(VERIFY_LINK_PAGE);
